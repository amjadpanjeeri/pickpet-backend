...
Like.create = (newLike, result) => {
  sql.query("INSERT INTO post_likes SET ?", newLike, (err, res) => {
    if (err) {
...
Like.checkLike = (post_id,user_id, result) => {
    sql.query("SELECT * FROM post_likes WHERE post_id = ? and user_id = ?", [post_id,user_id], (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
        return;
  ...
  Like.likeCount = (post_id, result) => {
    sql.query("SELECT COUNT(*) as count FROM post_likes WHERE post_id = ?", [post_id], (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
        return;
