...
Favourite.create = (newFavourite, result) => {
  sql.query("INSERT INTO saved_pet_accessories SET ?", newFavourite, (err, res) => {
    if (err) {
...
Favourite.getAll = (user_id, result) => {
  sql.query(
    "SELECT * from pet_accessories,saved_pet_accessories where pet_accessories.product_id = saved_pet_accessories.post_id and saved_pet_accessories.user_id = ?",
    [user_id],
    (err, res) => {
      if (err) {
        console.log("error: ", err);
...
Favourite.checkFavourite = (post_id,user_id, result) => {
  sql.query("SELECT * FROM saved_pet_accessories WHERE post_id = ? and user_id = ?", [post_id,user_id], (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
...
Favourite.remove = (user_id, post_id, result) => {
  sql.query(
    "DELETE FROM saved_pet_accessories WHERE user_id = ? and post_id = ?",
    [user_id, post_id],
    (err, res) => {
      if (err) {
        console.log("error: ", err);
